...
function registFile(e) {
	...
	if (filesArr.length > 0) {		
	    ...
	    filesArr.forEach(function(f){
			...
			$('#originalFileName').val(image_file_name);
...
function cancel_write() {
	swal({
		  ...
		  if (value == 'ok') {
              location.href = "/auth/community/board"
		  }
		})
}
...
function write_board() {
	...
	const formData = new FormData($('#registBoardForm')[0]);
	...
	if (regist_upload_file.length > 0) {
		for (var i = 0; i < regist_upload_file.length; i++) {	
			formData.append("file"+i, regist_upload_file[i]);
		}
	} 
	var aoo = ajax_formdata_options("POST", "/user/api/regist/board", formData);
	ajax(aoo, wbs_callback_function, wbf_callback_function)
}

// 
function wbs_callback_function(resp) {
	console.log(resp)
	if (resp.resultCode == 200) {
		swal('', '성공적으로 작성되었습니다.', 'success').then(function(){
			//location.href = '/auth/community/board/detail';
		});
		return;	
	} else {
...
// 
function wbf_callback_function(resp) {
	console.log(resp)
}
